...
class DB {
  ...
  public $last_query_type = 'WRITE';
  public static $read_host = 'localhost';
  public static $write_host = 'localhost';
...
class MeekroDB {
  ...
  public $last_query_type = 'READ';

  public $host = null; // will be replaced by read_host/write_host
  public $read_host = 'localhost';
  public $write_host = 'localhost';
  public $port = null;
  ...
  public function __construct($read_host=null, $write_host=null, $user=null, $password=null, $dbName=null, $port=null, $encoding=null) {
    if ($read_host === null) $read_host = DB::$read_host;
    if ($write_host === null) $write_host = DB::$write_host;
  ...
  public function get() {
    ...
    if (!($mysql instanceof MySQLi)) {
      ...
      if ($mysql->connect_error) {
        $this->nonSQLError('Unable to connect to server '.$this->host.' Error: ' . $mysql->connect_error);
      }
  ...
  protected function queryHelper() {
      ...
      // Select:
      // [0] => SELECT foo FROM test

      $write = true;
      if (stristr($args[0], 'SELECT') !== FALSE)
           $write = false;

      if ($write == true){
          if ($this->last_query_type != "WRITE"){
              $this->internal_mysql = null;
              $this->last_query_type = "WRITE";
              $this->host = $this->write_host;
          }
      }else{
          if ($this->last_query_type != "READ"){
              $this->internal_mysql = null;
              $this->last_query_type = "READ";
              $this->host = $this->read_host;
          }
      }
      $db = $this->get();
